...
import type { ContaReceberItem } from '@/mocks/contasReceber'
import { contasReceberSeed } from '@/mocks/contasReceber'

type Item = ContaReceberItem
const LS_KEY = 'contas_a_receber_v1'
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setItems(JSON.parse(raw))
        ...
        setItems(contasReceberSeed)
        localStorage.setItem(LS_KEY, JSON.stringify(contasReceberSeed))
      }
    } catch {
      setItems(contasReceberSeed)
    }
...
export default function ContasAReceber() {
  const { items, updateStatus } = useCRStore()

  // visibilidade dos filtros
  const [showFilters, setShowFilters] = useState(false)

  // filtros
  const [sacado, setSacado] = useState<string>('') // '' = todos
  const [from, setFrom] = useState<string>('')     // yyyy-mm-dd
  const [to, setTo] = useState<string>('')
  const [status, setStatus] = useState<string>('') // '' = todos

  // opções de sacados (dinâmico)
  const sacadosOpts = useMemo(() => {
    const map = new Map<string, string>()
    items.forEach((i) => {
      const id = i.sacadoId ?? i.sacadoNome ?? ''
      const nome = i.sacadoNome ?? i.sacadoId ?? ''
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((i) => {
      if (sacado) {
        const key = i.sacadoId ?? i.sacadoNome ?? ''
        if (key !== sacado) return false
      }
  ...
  }, [items, sacado, status, from, to])

  const total = useMemo(() => filtered.reduce((acc, i) => acc + (i.valor || 0), 0), [filtered])

  // tabela
  const headers = ['Descrição', 'Data', 'Valor (R$)', 'Status']
  ...
  const handleGerarPDF = () => {
    const title = 'Relatório - Contas a Receber'
    const filtroSacado = sacado
      ? (sacadosOpts.find(o => o.value === sacado)?.label ?? sacado)
      : 'Todos'
...
<body>
  ...
  <h1>${title}</h1>
  <div class="muted">
    Sacado: <strong>${filtroSacado}</strong> •
  ...
  <table>
    ...
    <tbody>
      ${rowsHtml || `<tr><td colspan="4" style="text-align:center;color:#666">Sem registros</td></tr>`}
    </tbody>
    <tfoot>
      <tr>
    ...
    <AdminLayout>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-3">
          {/* Lupa que mostra/oculta filtros */}
          <button
            type="button"
          ...
          {showFilters && (
            <div className="flex flex-wrap items-end gap-3 flex-1">
              <div className="flex flex-col">
                <label className="text-sm mb-1">Sacado</label>
                <select
