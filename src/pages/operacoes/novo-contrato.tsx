...
export default function NovoContrato() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (editandoId) {
      ...
      const { error } = await supabase.from('contratos').update({
        ...
        ...(anexoUrl ? { anexo_url: anexoUrl } : {}), // só atualiza se enviar novo arquivo
      }).eq('id', editandoId)
  ...
  const editar = (contrato: Contrato) => {
    setEditandoId(contrato.id)
    setCedenteId(contrato.cedente_id)
    setTipo(contrato.tipo)
    setLimite(contrato.limite)
    setMostrarForm(true)
  }
        ...
        {mostrarForm && (
          <div className="border rounded-xl p-4 bg-white">
            <h2 className="text-lg font-medium mb-3">
              {editandoId ? 'Editar contrato' : 'Cadastrar contrato'}
            </h2>
